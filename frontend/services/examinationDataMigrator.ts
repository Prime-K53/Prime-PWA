...
const migrateStore = async <T extends { id: string }>(
  ...
  try {
    const allItems = await (dbService as any).getAll(storeName) as T[];
...
const migrateBatchesFromOfflineDb = async (): Promise<number> => {
  try {
    ...
    await examinationDb.examinationBatches.bulkPut(toInsert as any);
    return toInsert.length;
  } catch (error) {
