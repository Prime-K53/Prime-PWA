...
export const enrichInvoiceWithBatchPricing = (
  ...
  const roundingTotal = roundingAdjustmentTotal !== 0
    ...
    : toNumber(
        ...
        ?? (invoice as any).rounding_total
        ?? (invoice as any).rounding_difference,
        0
      );
  ...
  const items = classes.map((cls: any, index: number) => {
    ...
    return {
      ...
      pricingBreakdown: {
        ...
        roundingMethod: String(invoice.roundingMethod || (invoice as any).rounding_method || batch?.rounding_method || 'nearest_50'),
