...
router.get('/profit-margins', async (req, res) => {
  try {
    ...
    const settings = await profitMarginService.listSettings(scope, req.companyId || '');
    res.json(settings);
  } catch (err) {
...
router.get('/profit-margins/:id', async (req, res) => {
  try {
    const setting = await profitMarginService.getById(req.params.id, req.companyId || '');
...
router.post('/profit-margins', authorizePricing, validateBody(profitMarginSchemas.create), auditCrudMiddleware('profit_margin_setting'), async (req, res) => {
  try {
    ...
    const result = await profitMarginService.createSetting(req.body, userId, req.companyId || '');
...
router.patch('/profit-margins/:id', authorizePricing, validateBody(profitMarginSchemas.update), auditCrudMiddleware('profit_margin_setting'), async (req, res) => {
  try {
    const userId = req.user?.id || 'system';
    const result = await profitMarginService.updateSetting(req.params.id, req.body, userId, req.companyId || '');
    res.json(result);
  } catch (err) {
...
router.delete('/profit-margins/:id', authorizePricing, auditCrudMiddleware('profit_margin_setting'), async (req, res) => {
  try {
    ...
    await profitMarginService.deleteSetting(req.params.id, userId, reason || 'User requested deletion', req.companyId || '');
...
router.post('/profit-margins/bulk-upload', authorizePricing, validateBody(profitMarginSchemas.bulkUpload), async (req, res) => {
  try {
    ...
    const results = await profitMarginService.bulkUpload(rows, userId, req.companyId || '');
    res.json(results);
  } catch (err) {
